# Contributing to @skalfa/skalfa-component
...
* **Component Naming**: Follow the `ComponentNameComponent` convention (e.g. `ButtonComponent`, `ModalComponent`).
* **Testing**: Verify that `npm run build` compiles with 0 errors before submitting a PR.
* **No Accidental Build Artifacts**: Never commit `.js` or `.d.ts` files to the `src/` directory. Always run `npm run build` (not `tsc` directly) to output to `dist/`.
